...
int displayServer() {
    ...
    // The images (X, O and Black) are assumed to be square and all the same size
    ...
    sf::RenderWindow window(sf::VideoMode(windowSize, windowSize), "Tic-Tac_Toe (tahsinn1 Display Server)", sf::Style::Titlebar | sf::Style::Close);
    ...
    sf::RectangleShape horizontal(sf::Vector2f(barHeight, barWidth));
    sf::RectangleShape vertical(sf::Vector2f(barWidth, barHeight));
    horizontal.setFillColor(sf::Color::Black);
    vertical.setFillColor(sf::Color::Black);
    // horizontal
    horizontal.setPosition(0,tileSize);
    window.draw(horizontal);
    horizontal.setPosition(0,2*tileSize+barWidth);
    window.draw(horizontal);
    // vertical
    vertical.setPosition(tileSize,0);
    window.draw(vertical);
    vertical.setPosition(tileSize*2+barWidth,0);
    window.draw(vertical);
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left && !game_state.get_gameOver()) {
                ...
                game_state.set_clickedRow(xPosition/tileSize);
                game_state.set_clickedColumn(yPosition/tileSize);
        ...
        for (int i = 0; i < boardSize; ++i) {
            for (int j = 0; j < boardSize; ++j) {
                if (game_state.get_gameBoard(i,j)==X){
                    ...
                    X_sprite.setPosition((tileSize+barWidth)*i,(tileSize+barWidth)*j);
                }
                else if (game_state.get_gameBoard(i,j)==O) {
                    window.draw(O_sprite);
                    O_sprite.setPosition((tileSize+barWidth)*i,(tileSize+barWidth)*j);
                }
                else {
                    window.draw(Blank_sprite);
                    Blank_sprite.setPosition((tileSize+barWidth)*i,(tileSize+barWidth)*j);
                }
            }
        }
        
        
        sf::RectangleShape horizontalWin(sf::Vector2f(barHeight, barWidth));
        sf::RectangleShape verticalWin(sf::Vector2f(barWidth, barHeight));
        sf::RectangleShape diagonalWin(sf::Vector2f(barWidth, 1.41421*barHeight));
        horizontal.setFillColor(sf::Color::White);
        vertical.setFillColor(sf::Color::White);
        diagonalWin.setFillColor(sf::Color::White);
        
        if (game_state.get_winCode()==1) {
            verticalWin.setPosition((tileSize+barWidth)/2,0);
            window.draw(verticalWin);
        } 
        else if (game_state.get_winCode()==2) {
            verticalWin.setPosition(((3*tileSize+barWidth)/2),0);
            window.draw(verticalWin);
        }
        else if (game_state.get_winCode()==3) {
            verticalWin.setPosition(((5*tileSize+barWidth)/2),0);
            window.draw(verticalWin);
        }
        else if (game_state.get_winCode()==4) {
            horizontalWin.setPosition(0,(tileSize+barWidth)/2);
            window.draw(horizontalWin);
        }
        else if (game_state.get_winCode()==5) {
            horizontalWin.setPosition(0,((3*tileSize+barWidth)/2));
            window.draw(horizontalWin);
        }
        else if (game_state.get_winCode()==6) {
            horizontalWin.setPosition(0,((5*tileSize+barWidth)/2));
            window.draw(horizontalWin);
        }
        else if (game_state.get_winCode()==7) {
            diagonalWin.setPosition(0,0);
            diagonalWin.rotate(-45);
            window.draw(diagonalWin);
        }
        else if (game_state.get_winCode()==8) {
            diagonalWin.setPosition(windowSize,0);
            diagonalWin.rotate(45);
            window.draw(diagonalWin);
        }
        else {}
